...
class Crawler:
    ...
    def get_chapter(self, chapter_param):
        ...
        soup = BeautifulSoup(site_rs.content, 'html.parser')
        book_title_tag = soup.find('span', class_='chuto40')
        if book_title_tag:
            book_title = book_title_tag.string.strip()

        # get book_thumb from css
        book_thumb_tag = soup.find('div', id='anhbia')
        if book_thumb_tag:
        ...
        # get book_author
        # get chapter title
        chutieude_tags = soup.find_all('span', class_='chutieude')
        chutieude_list = []
        ...
        # get chapter content
        content_re = re.search('(<div id="chuhoain"(\w|\W)*) --!!tach_noi_dung!!--', site_rs.content)
        chapter_content = content_re.group(1)
